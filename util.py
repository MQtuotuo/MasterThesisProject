...
def save_history(history, prefix):
    ...
    if 'mean_absolute_error' not in history.history:
        return
...
def set_samples_info():
    ...
    dirs_info = {config.train_male_dir: 0, config.validation_male_dir: 0}
    for d in dirs_info:
    ...
    config.nb_train_samples = dirs_info[config.train_male_dir]
    config.nb_validation_samples = dirs_info[config.validation_male_dir]


def get_class_weight(d):
    white_list_formats = {'png', 'jpg', 'jpeg', 'bmp'}
    class_number = dict()
    dirs = sorted([o for o in os.listdir(d) if os.path.isdir(os.path.join(d, o))], key=int)
    k = 0
